...
[[ $var =~ ^regex$ ]]：正则表达式匹配。
 ...
 ------------------------------------------------------------------
练习：  ，执行脚本时可以带上任意的参数，参数都是数字。
无论带多少个参数，请计算从1到这些数字的累加。要求使用自定义变量来完成，
...
新建一个shell脚本
练习1：创建两个数组变量arr1、arr2，
arr1包含两个姓氏{yan,li}，arr2包含三个名字（不含姓）{alen,bob,candy}
，
使用两个姓氏和三个中文名拼接出名字来，通过数组实现，一共六个姓名。
写成脚本，执行脚本将名字输出。 
