...
# 流程规范:
# 1. 遍历 Table_Models/DWS/ 下的模型配置
# 2. 调用 infra 解析工具生成 ORC 建表语句并保底建表
# 3. 寻找 ETL/sql/dws/ 下同名的 .sql 模板，注入环境变量并执行数据装载
###
...
LOG_FILE="${LOG_DIR}/dwd_to_dws_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOG_FILE") 2>&1

DWS_DB="${HIVE_DATABASE:-mall2}"
DWS_CONF_DIR="${PROJECT_DIR}/Table_Models/DWS/$MALL_MODEL_DIR"
SQL_TEMPLATE_DIR="${PROJECT_DIR}/ETL/sql/dws"
SUCCESS_COUNT=0
FAIL_COUNT=0
...
process_dws_table() {
    local conf_file=$1
    echo "------------------------------------------"
    echo "[INFO] 开始处理 DWS 模型配置: $(basename "$conf_file")"
    ...
    for dwd_tbl in $dependent_dwd_tables; do
        if ! check_hive_table "$DWS_DB" "$dwd_tbl"; then
            echo "[ERROR] 严重阻断: 缺失依赖的上游表 ${DWS_DB}.${dwd_tbl}，终止处理 $target_table！"
            return 1
        fi
    done
    echo "[INFO] 上游依赖校验全部通过。"

    # 3.2 校验下游 DWS 表是否存在以决定是否跳过 DDL
    if check_hive_table "$DWS_DB" "$target_table"; then
        echo "[INFO] 下游目标表 ${DWS_DB}.${target_table} 已存在，跳过自动建表阶段。"
    else
        ...
        if [[ "$CREATE_CONFIRM" == "y" || "$CREATE_CONFIRM" == "Y" ]]; then
            ...
            generate_ddl_from_conf "$conf_file" "$DWS_DB" || return 1
    ...
    hive -hiveconf app_db="$DWS_DB" -hiveconf do_date="$DO_DATE" -f "$sql_file"
